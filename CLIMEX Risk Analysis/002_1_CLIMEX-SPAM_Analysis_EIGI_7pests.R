...
# ##
# #Add in region info
# #Rust Affected Area by Region
#
# #High input + Irrigation areas
# Region_Rust_hi <- SPAM_CX_Zone[, TotWhea_h:= whea_h+whea_i
#                             ][, .(TotWhea=sum(whea, na.rm=TRUE), TotWhea_h=sum(TotWhea_h, na.rm=TRUE),
#                                   S=sum(dS*TotWhea_h, na.rm=TRUE),
#                                   Y=sum(dY*TotWhea_h, na.rm=TRUE),
#                                   L=sum(dL*TotWhea_h, na.rm=TRUE),
#                                   SY=sum(dSY*TotWhea_h, na.rm=TRUE),
#                                   YL=sum(dYL*TotWhea_h, na.rm=TRUE),
#                                   LS=sum(dLS*TotWhea_h, na.rm=TRUE),
#                                   SYL=sum(dSYL*TotWhea_h, na.rm=TRUE)
#                             ), by=region]
# write.csv(Region_Rust_hi, paste0("./Final Data/Region_Rust_hi_", EG.sel, ".csv"))
#
#
# #All wheat areas
# Region_Rust_all <- SPAM_CX_Zone[, .(TotWhea=sum(whea, na.rm=TRUE),
#                                   S=sum(dS*whea, na.rm=TRUE),
#                                   Y=sum(dY*whea, na.rm=TRUE),
#                                   L=sum(dL*whea, na.rm=TRUE),
#                                   SY=sum(dSY*whea, na.rm=TRUE),
#                                   YL=sum(dYL*whea, na.rm=TRUE),
#                                   LS=sum(dLS*whea, na.rm=TRUE),
#                                   SYL=sum(dSYL*whea, na.rm=TRUE)
#                                   ), by=region]
#
# write.csv(Region_Rust_all, paste0("./Final Data/Region_Rust_all_", EG.sel, ".csv"))
...
# ###############################################################################
# aggtable <- read.csv("./Zone table/cck_aggtable.csv", stringsAsFactors=FALSE)
# 
# substReg <- function(old, newname) {
#   if(sum(aggtable$region==old) == 0) {
#     stop("non matching region")
#   }
#   aggtable[aggtable$region==old,]$region  <- newname
#   return(aggtable)
# }
# 
# 
# if(TRUE) {
#   #SSA
#   aggtable  <-  substReg("ECentralAfrica", "SSA")
#   aggtable  <-  substReg("SouthAfr", "SSA")
#   aggtable  <-  substReg("WCentralAfrica", "SSA")
# 
#   #China, India
#   aggtable[aggtable$ccode == 351, ]$region  <-  "China"
#   aggtable[aggtable$ccode == 100, ]$region  <-  "India"
#   aggtable[aggtable$ccode == 10, ]$region  <-  "Australia"
#   aggtable[aggtable$ccode == 249, ]$region  <-  "Yemen"
#   aggtable[aggtable$ccode == 136, ]$region  <-  "Mauritania"
#   aggtable[aggtable$ccode == 205, ]$region  <-  "Western Sahara"
#   aggtable[aggtable$ccode == 231, ]$region  <-  "United States"
# 
#   #w.europe
#   notEu <- c("Algeria",       "Tunisia",        "Libyan Arab Jamahiriya", "Morocco")
#   aggtable[aggtable$region=="NAfrWEu" & !aggtable$country %in% notEu,]$region <- "WEu"
#   aggtable[aggtable$region=="NAfrWEu",]$region  <- "NAfr"
# 
# 
# }
# 
# dat <- merge(SPAM_CX_Zone, aggtable, by.x="FAOCODE", by.y="ccode", all.x=TRUE)
# warning("Need to check merge, how much is missed?")
# 
# ##
# #Add in region info
# #Rust Affected Area by Region
# 
# #High input + Irrigation areas
# Region_Pest_hi <- dat[, TotWhea_h:= whea_h+whea_i
#                                ][, .(TotWhea=sum(whea, na.rm=TRUE), TotWhea_h=sum(TotWhea_h, na.rm=TRUE),
#                                      pg=sum(dpg*TotWhea_h, na.rm=TRUE),
#                                      ps=sum(dps*TotWhea_h, na.rm=TRUE),
#                                      pt=sum(dpt*TotWhea_h, na.rm=TRUE)), by=region.y]
# write.csv(Region_Pest_hi, paste0("./Final Data/Region_Pest_hi_", HP.sel, "_", EG.sel, ".csv"))
# 
# 
# #All wheat areas
# Region_Pest_all <- dat[, .(TotWhea=sum(whea, na.rm=TRUE),
#                                     pg=sum(dpg*whea, na.rm=TRUE),
#                                     ps=sum(dps*whea, na.rm=TRUE),
#                                     pt=sum(dpt*whea, na.rm=TRUE)), by=region.y]
# 
# write.csv(Region_Pest_all, paste0("./Final Data/Region_Pest_all_", HP.sel, "_", EG.sel, ".csv"))
